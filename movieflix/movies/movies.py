...
import movieflix.utilities.utilities as utilities
import movieflix.movies.services as services

from movieflix.authentication.authentication import login_required
...
def movies_by_year():
    ...
    if target_year is None:
        ...
        # Convert target_year from string to date.
...
# wtf bro
    if len(movies) > 0:
        ...
        if previous_year is not None:
            ...
            prev_movie_url = url_for('movies_bp.movies_by_year', release_year=previous_year.isoformat())
            first_movie_url = url_for('movies_bp.movies_by_year', release_year=first_movie['release_year'].isoformat())

        # There are movies on a subsequent date, so generate URLs for the 'next' and 'last' navigation buttons.
        if next_year is not None:
            next_movie_url = url_for('movies_bp.movies_by_year', release_year=next_year.isoformat())
            last_movie_url = url_for('movies_bp.movies_by_year', release_year=last_movie['release_year'].isoformat())

        # Construct urls for viewing movie reviews and adding reviews.
        for movie in movies:
            movie['view_review_url'] = url_for('movies_bp.movies_by_year', release_year=target_year, view_reviews_for=movie['id'])
...
def movies_by_tag():
    ...
    if cursor > 0:
        ...
        prev_movie_url = url_for('movies_bp.movies_by_tag', tag=tag_name, cursor=cursor - movies_per_page)
        first_movie_url = url_for('movies_bp.movies_by_tag', tag=tag_name)

    if cursor + movies_per_page < len(movie_ids):
        # There are further movies, so generate URLs for the 'next' and 'last' navigation buttons.
        next_movie_url = url_for('movies_bp.movies_by_tag', tag=tag_name, cursor=cursor + movies_per_page)

        last_cursor = movies_per_page * int(len(movie_ids) / movies_per_page)
        if len(movie_ids) % movies_per_page == 0:
            last_cursor -= movies_per_page
        last_movie_url = url_for('movies_bp.movies_by_tag', tag=tag_name, cursor=last_cursor)

    # Construct urls for viewing movie reviews and adding reviews.
    for movie in movies:
        movie['view_review_url'] = url_for('movies_bp.movies_by_tag', tag=tag_name, cursor=cursor, view_reviews_for=movie['id'])
        movie['add_review_url'] = url_for('movies_bp.review_on_movie', movie=movie['id'])
...
def review_on_movie():
    ...
    if form.validate_on_submit():
        ...
        services.add_review(movie_id, form.review.data, user_name, repo.repo_instance)
